defmodule TesmoinWeb.UserAuthTest do
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "uses persisted current store when session current_store_id is missing", %{
      ...
      {:ok, _updated_user} = Accounts.set_current_store(user, store2.id)
  ...
  describe "redirect_if_user_is_authenticated/2" do
    ...
    test "does not redirect in re-authentication mode", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> fetch_flash()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TesmoinWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
