defmodule TesmoinWeb.InvitationLive do
  ...
  def mount(%{"token" => token}, _session, socket) do
    ...
    cond do
      ...
      not is_nil(invitation.accepted_at) ->
        {:ok, assign(socket, state: :already_accepted, invitation: invitation)}

      DateTime.compare(invitation.expires_at, DateTime.utc_now(:second)) == :lt ->
        {:ok, assign(socket, state: :expired, invitation: invitation)}

      true ->
        {:ok, assign(socket, state: :pending, invitation: invitation)}
    end
  end
  ...
  def handle_event("accept", _params, %{assigns: %{state: :pending}} = socket) do
    ...
    case RateLimiter.check_magic_link_request(client_ip) do
      :rate_limited ->
        {:noreply, put_flash(socket, :error, "Too many requests. Please wait a minute and try again.")}
            ...
            <div class="backoffice-card flex flex-col items-center gap-6 px-8 py-12 text-center">
              ...
              <div>
                ...
                <p class="mt-2 text-sm text-slate-500 leading-relaxed">
                  {if @invitation.invited_by, do: @invitation.invited_by.email, else: "A Tesmoin administrator"}
                  has invited you to join this Tesmoin node as
                  <span class="font-semibold text-slate-700">{String.capitalize(@invitation.role)}</span>.
                </p>
