defmodule Tesmoin.Accounts do
  ...
  alias Tesmoin.Accounts.{User, UserToken, UserNotifier}
  ...
  """
  def set_current_store(%User{} = user, store_id) when is_integer(store_id) do
    user
  ...
  See `Tesmoin.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  """
  def generate_login_token(%User{} = user, magic_link_url_fun)
      when is_function(magic_link_url_fun, 1) do
    {encoded_token, user_token} = UserToken.build_email_token(user, "login")
    Repo.insert!(user_token)
